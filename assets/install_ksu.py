...
def install_ksu(lang_code="pt"):
    ...
    subprocess.run(["adb", "shell", "cd /data/local/tmp && sh -x ksupatcher.sh ksu"], check=False)
    ...
    try:
        ...
        patched_files = [f for f in files if f.startswith("kernelsu_patched_") and f.endswith(".img")]
        if patched_files:
            patched_files.sort()
